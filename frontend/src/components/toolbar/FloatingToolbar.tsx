...
import { Clock, Wallet, GitBranch, Zap, Upload, Download, Brain, Trash2, Sparkles, Menu, Undo2, Redo2, Save } from "lucide-react";
...
export function FloatingToolbar({ strategy, onBlockAdd, onStrategyLoad, onStrategySave, onTemplateLoad, onReset, onAutoLayout, onUndo, onRedo, canUndo, canRedo, isSaving }: FloatingToolbarProps) {
  ...
  const blocks = [
    { type: BlockType.ASSET, icon: Wallet, label: "Portfolio" },
    { type: BlockType.CONDITION, icon: GitBranch, label: "Trigger" },
    { type: BlockType.ACTION, icon: Zap, label: "Execute" },
  ];
      ...
      <div className="absolute top-5 left-1/2 -translate-x-1/2 z-50">
        <div className="bg-white rounded-lg shadow-lg border border-gray-300 px-3 py-2.5 flex items-center gap-3 min-w-max">
          ...
          <button
            ...
            title="Go to Home"
          >
            <div className="w-6 h-6 bg-gradient-to-br from-[hsl(174,100%,45%)] to-[hsl(270,80%,60%)] rounded flex items-center justify-center shadow-sm">
              <Brain className="w-3.5 h-3.5 text-white" />
            </div>
            <span className="text-sm font-bold bg-gradient-to-r from-[hsl(174,100%,45%)] to-[hsl(270,80%,60%)] bg-clip-text text-transparent">DeleGator.AI</span>
          </button>
